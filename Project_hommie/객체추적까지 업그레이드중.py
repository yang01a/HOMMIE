...
import subprocess
import gc
from geometry_msgs.msg import Twist  # ROS에서 로봇의 움직임을 제어하는 메시지
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            if Class == 0 and confidence > 70:  # Class 0 is 'person' in COCO
                ...
                person_location = (x1 + x2) // 2, (y1 + y2) // 2  # Track the center of the person
...
def follow_person():
    ...
    if person_location is not None:
        center_x, center_y = person_location
        frame_width = 640  # Assuming the frame width is 640 pixels
        frame_height = 480  # Assuming the frame height is 480 pixels

        # Calculate the error in x and y direction
        error_x = center_x - frame_width // 2
        error_y = center_y - frame_height // 2

        # Create Twist message for controlling robot
        move_cmd = Twist()

        # Move the robot based on the error
        if abs(error_x) > 20:
            move_cmd.linear.x = 0.1  # Move forward if person is not centered
            move_cmd.angular.z = -0.01 * error_x  # Adjust rotational speed based on the error
        else:
            move_cmd.linear.x = 0.0  # Stop forward movement

        if abs(error_y) > 20:
            move_cmd.angular.z = -0.01 * error_y  # Adjust rotation to align vertically
    ...
    while not rospy.is_shutdown():
        if frame is not None:
            ...
            try:
                ...
                follow_person()  # Call to follow the person if detected
            except Exception as e:
                print(f"Error in person detection or following: {e}")
            ...
            try:
                ...
                print(f"Error in MJPEG encoding: {e}")
...
def start_patrol():
    ...
    try:
        # ROS 환경 변수 설정
        env = os.environ.copy()
        env["ROS_MASTER_URI"] = "http://192.168.123.5:11311"  # ROS Master URI 설정
